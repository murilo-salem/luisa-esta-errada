...
As métricas de classificação foram implementadas para oferecer uma avaliação global, ponderada e por classe, com ênfase especial no comportamento da classe maligna. Além das métricas agregadas, o pipeline gera matriz de confusão, curvas ROC \textit{one-vs-rest} e um arquivo de predições contendo probabilidades por classe para cada imagem.

\subsubsection{Accuracy}

A \textit{accuracy} mede a proporção global de predições corretas. Embora seja intuitiva e útil como visão geral, sua interpretação isolada é insuficiente em cenários desbalanceados, pois pode mascarar desempenho insatisfatório em classes menos frequentes.

\subsubsection{Precision}

A \textit{precision} quantifica a proporção de predições positivas corretas. O pipeline registra \texttt{macro precision}, \texttt{weighted precision} e \textit{precision} por classe, o que permite analisar tanto desempenho médio quanto comportamento específico de cada rótulo.

\subsubsection{Recall / Sensibilidade}

O \textit{recall}, ou sensibilidade, mede a capacidade de recuperar corretamente os exemplos positivos. No problema estudado, o \texttt{malignant\_recall} recebe atenção especial, por refletir diretamente a sensibilidade do modelo à classe clinicamente mais crítica.

\subsubsection{F1-score}

O \(F_1\)-\textit{score} combina \textit{precision} e \textit{recall} em uma média harmônica. O pipeline utiliza o \texttt{macro\_f1} como métrica primária de seleção na classificação, justamente por reduzir o risco de favorecer artificialmente a classe majoritária.

\subsubsection{AUC-ROC}

A AUC-ROC é calculada em esquema \textit{one-vs-rest}, tanto em versão macro quanto ponderada. Essa métrica é importante porque avalia a separabilidade entre classes independentemente de um único limiar de decisão.

\subsubsection{Especificidade}

A especificidade é registrada por classe e mede a capacidade do modelo de identificar corretamente os casos negativos em relação a um rótulo considerado. Sua presença complementa a análise de sensibilidade, especialmente em problemas médicos.

\subsection{Métricas de segmentação}

As métricas de segmentação são calculadas a partir das contagens binárias por pixel: verdadeiros positivos, falsos positivos, falsos negativos e verdadeiros negativos. O pipeline registra métricas agregadas, métricas por imagem, métricas por classe e estatísticas estruturais relacionadas à quantidade de pixels positivos previstos e observados.

\subsubsection{IoU}

A \textit{Intersection over Union} (IoU) mede a razão entre a interseção e a união das máscaras predita e de referência. É uma métrica de sobreposição espacial altamente informativa para qualidade segmentadora global.

\subsubsection{Dice Score}

O coeficiente Dice é a métrica primária de seleção na segmentação. Por ser particularmente adequado a cenários com desbalanceamento entre lesão e fundo, ele ocupa papel central no protocolo experimental.

\subsubsection{Precision}

A \textit{precision} em segmentação mede a proporção de pixels previstos como positivos que realmente pertencem à lesão. É útil para caracterizar tendência à sobresegmentação.

\subsubsection{Recall}

O \textit{recall} mede a capacidade do modelo de recuperar os pixels verdadeiramente pertencentes à lesão. É importante para identificar subsegmentação.

\subsubsection{F1-score pixel-wise}

O \(F_1\)-\textit{score} pixel-wise sintetiza o equilíbrio entre \textit{precision} e \textit{recall} em nível de pixel, servindo como medida complementar às métricas de sobreposição.

\subsection{Métricas computacionais}

As métricas computacionais foram tratadas como parte integrante da avaliação, e não como análise secundária. O pipeline registra contagens de parâmetros, tempo total de treinamento, tempo médio por época, pico de memória em GPU, latência média de inferência, desvio-padrão de latência, \textit{throughput} e tamanho final do modelo.

\subsubsection{Número de parâmetros}

São registrados \texttt{parameters\_total} e \texttt{parameters\_trainable}. Essa distinção é especialmente importante em cenários com \textit{transfer learning}, pois diferentes estratégias de ajuste fino alteram substancialmente a parcela efetivamente treinável da arquitetura.

\subsubsection{Tempo de treinamento}

O pipeline registra o tempo total por \textit{trial} e o tempo médio por época, o que permite discutir custo de treinamento em termos empíricos, e não apenas em função do tamanho da rede.

\subsubsection{Tempo de inferência}

A latência média e seu desvio-padrão são obtidos por \textit{benchmark} dedicado com etapas de aquecimento e medição. Isso permite avaliar viabilidade prática de cada modelo em cenários de uso real.

\subsubsection{Tamanho do modelo}

O tamanho final do modelo em megabytes é registrado nos resumos de arquitetura e compõe a análise de compromisso entre desempenho preditivo e viabilidade de armazenamento e implantação.
