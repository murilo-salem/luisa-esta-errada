...
A etapa final de classificação contemplou quatro arquiteturas representativas de diferentes paradigmas de projeto em redes neurais convolucionais: VGG16, ResNet50, EfficientNet-B0 e ConvNeXt-Large. Essa escolha permitiu comparar, em um mesmo ambiente experimental, um modelo clássico de alta capacidade paramétrica, uma arquitetura residual profunda, uma rede explicitamente orientada à eficiência e uma CNN modernizada de grande porte. Em todos os casos, a avaliação final foi realizada sobre o mesmo conjunto de teste, o que garante comparabilidade direta entre os resultados reportados.

Os artefatos finais associam a cada arquitetura um \textit{trial} considerado ótimo dentro do espaço de busca adotado. Para a VGG16, o melhor \textit{trial} foi \texttt{trial\_11\_lr\_0.0001\_bs\_32\_ft\_partial}; para a ResNet50, \texttt{trial\_03\_lr\_0.001\_bs\_16\_ft\_deep}; para a EfficientNet-B0, \texttt{trial\_05\_lr\_0.001\_bs\_32\_ft\_partial}; e, para a ConvNeXt-Large, \texttt{trial\_05\_lr\_0.001\_bs\_32\_ft\_partial}. Esses resultados mostram que a configuração final não foi uniforme entre os modelos, o que é esperado em uma comparação justa: cada arquitetura respondeu melhor a combinações específicas de taxa de aprendizado, tamanho de lote e profundidade de ajuste fino.

Em termos de capacidade arquitetural, as diferenças entre os modelos foram substanciais. A ConvNeXt-Large foi o modelo mais pesado, com 196.234.947 parâmetros totais, enquanto a EfficientNet-B0 foi, de longe, a mais compacta, com 4.011.391 parâmetros. A ResNet50 ocupou uma posição intermediária, com 23.514.179 parâmetros, ao passo que a VGG16 manteve custo paramétrico elevado, com 134.272.835 parâmetros. Essa heterogeneidade é relevante porque mostra que os resultados de desempenho absoluto precisam ser interpretados em conjunto com custo computacional, e não apenas como competição cega de métricas finais.
...
A comparação entre arquiteturas mostra um cenário tecnicamente interessante. A ResNet50 e a ConvNeXt-Large atingiram exatamente a mesma \textit{accuracy} final (\(0{,}8729\)), mas a ResNet50 apresentou vantagem em macro-\textit{precision}, macro-\(F_1\), macro-AUC, latência de inferência e custo paramétrico. Isso indica que a igualdade em acurácia esconde comportamentos bastante diferentes: enquanto a ConvNeXt-Large alcança forte capacidade de recuperação global, a ResNet50 produz melhor equilíbrio entre precisão, separabilidade e eficiência.

A EfficientNet-B0 ocupou posição intermediária em desempenho, mas destacou-se fortemente em eficiência. Seu número total de parâmetros foi quase seis vezes menor que o da ResNet50 e quase cinquenta vezes menor que o da ConvNeXt-Large, com latência de inferência também baixa. Essa combinação explica por que, mesmo não sendo o melhor modelo absoluto, ela se torna uma candidata natural ao melhor compromisso entre desempenho e custo computacional. Em aplicações de apoio ao diagnóstico com restrição de recursos, essa característica pode ser mais relevante do que pequenos ganhos marginais em métrica final.
...
Considerando o conjunto completo de métricas globais, a ResNet50 foi o melhor modelo em desempenho absoluto na tarefa de classificação. O resumo consolidado da execução \texttt{artifacts} já a identifica como melhor arquitetura absoluta, com macro-\(F_1 = 0{,}8710\) e AUC = \(0{,}9744\), além de apontá-la como melhor modelo em sensibilidade para malignidade, com \textit{recall} de \(0{,}9688\). No detalhamento por modelo, a ResNet50 também alcançou \textit{accuracy} de \(0{,}8729\), macro-\textit{precision} de \(0{,}8673\) e macro-\textit{recall} de \(0{,}8840\), consolidando o melhor equilíbrio geral entre acerto, capacidade discriminativa e sensibilidade clínica.
...
\begin{itemize}
	\item U-Net++: \(26{,}1\) milhões;
	\item Attention U-Net: \(24{,}4\) milhões;
	\item U-Net: \(24{,}4\) milhões;
	\item DeepLabV3+: \(22{,}4\) milhões.
\end{itemize}
...
Na inferência, a DeepLabV3+ foi a mais rápida, com \(1{,}82\) ms por imagem. Em seguida vieram U-Net (\(1{,}89\) ms/img), Attention U-Net (\(2{,}17\) ms/img) e U-Net++ (\(2{,}71\) ms/img). Isso mostra que a U-Net++ pagou um custo operacional moderado por sua superioridade em qualidade segmentadora.

A diferença, contudo, não foi extrema em termos absolutos, o que preserva a viabilidade prática da U-Net++ em muitos cenários operacionais.
